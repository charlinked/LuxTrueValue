...
'''
Scrapes all overview pages in a loop for Louis Vuitton bags - 23 pages currently
CURRENT Problem: with this script results = 0 although soup has content 
The normal link collections/bags/brand doesn't seem to work for LV
- Either try another link ?
- double check that the element is present in the soup and it's still the same tag:
results = soup.find_all('div',attrs={'class': 'product-caption'}) 
- try if it works with the normal link - why LV different
'''
...
for i in range(1,2): 
    pagenr = str(i)
    urlpage = 'https://shop.rebag.com/search?page='+pagenr+'&q=louis+vuitton&pf_st_availability_hidden=true'
    print(urlpage)
    ...
    for i in range(len(results)):
        ...
        model = getcontentout('Louis Vuitton Model: ',' Exterior Material:')  #change for other designers!
...
with open('rebag_LV.csv','w', newline='') as f_output:
