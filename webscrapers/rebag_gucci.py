...
'''
Scrapes all overview pages in a loop for Gucci bags - 16 pages currently
'''

bags = []
bags.append(['productname','brand','price','pagelink','condition','conditiondetails','exteriorcolor','exteriormaterial','hardware','accessories','brandcode', 'measurements','itemnr','claircode','fulldescr'])

for i in range(1,17): 
    pagenr = str(i)
    urlpage = 'https://shop.rebag.com/collections/Gucci/?page='+pagenr
    print(urlpage)
    ...
    for i in range(len(results)):
        ...
        model = getcontentout('Gucci Model: ',' Exterior Material:')  #change for other designers!
        ...
        try:
            fulldescr= ((description_str.split('"description": "')[1]).split('. These are professional pictures'))[0]
        except:
            fulldescr= None

        #append list with info of each bag
        bags.append([model,brand,price,page2,condition,conditiondet,extcolor,extmat,hardwarecolor,acc,authcode,measurem,itemnr,claircode,fulldescr])
        print(bags[-1])

#print(bags)

# Create csv and write bags list to output file
with open('rebag_gucci.csv','w', newline='') as f_output:
